...
import { FixturePill, PlayerVisual, TeamIdentity } from '@/components/team-visuals';
...
function PlayerDrawer({ player, onClose }: { player: PlayerView | null; onClose: () => void }) {
...
function SquadView({ data, selectPlayer }: { data: DashboardData; selectPlayer: (player: PlayerView) => void }) {
  const [mode, setMode] = useState<'xpts' | 'live' | 'price'>('xpts');
